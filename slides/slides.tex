...
\item Creación de tablas, especificando las {\em column families}
  iniciales:
\begin{lstlisting}[language=ruby]
hbase> create 'productos', 'd', 'val'
0 row(s) in 1.3810 seconds
=> Hbase::Table - slides
\end{lstlisting}

\item Se crea la tabla {\tt productos} con dos familias de columnas,
  {\tt d}, que guardará los datos del producto, y {\tt val}, que
  guardará las valoraciones de los usuarios

\item Se puede insertar datos y consultar desde el {\em shell\/}, pero lo
  veremos desde Python
\end{itemize}
\end{frame}
...
\item También aplicamos al parámetro {\tt bloom\_filter\_type} el valor
  {\tt ROW} \framebreak
